...
const AlbumItem = ({ item, index, control }) => {
  ...
  return (
    <div className="item" key={index}>
      ...
      <span className="song-name">
        <Link key={index} className="song" to={`/song/${item.id}`} dangerouslySetInnerHTML={{__html: item.title}}/>
      </span>

      <span className="artists">
        {item.more_info.artistMap.artists.map((artist, index) => {
          return (
            <Link key={index} className="artist" to={`/artist/${artist.id}`}>{artist.name},&nbsp;</Link>
          );
        })}
      </span>
      ...
      <span className="duration">
        ...
        {("0" + Math.floor(item.more_info.duration % 60)).slice(-2)}
      </span>
    </div>
  );
};
